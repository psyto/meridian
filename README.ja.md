...
4. キーパーがJupiterスワップを実行し出力額を提案（990 wSOL）。**独立した再実行 attestor が同一スワップを LiteSVM で cloned mainnet 状態に対し再実行し、出力が一致した時のみ `execute_swap` に co-sign**（キーパー単独では捏造した出力を確定できない）
5. トレーダーが987.03 wSOLを引き出し（2.97 wSOL手数料控除後、最低額はネット額でオンチェーン検証）

**復活（revival）による変更:** キーパー単独信頼 → キーパー＋独立 attestor の二者モデル。`execute_swap` は attestor 署名を必須化。off-chain の attestor は custos-engine で再実行し ed25519 で署名（`services/attestor`、詳細は [`REVIVAL.md`](./REVIVAL.md)）。本番では detached ed25519 attestation のオンチェーン検証（slice 1b）が続く。
